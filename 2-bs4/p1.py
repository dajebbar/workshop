...
book_price = [price.find('p',{'class':'price_color'}).get_text().replace('£','').strip() for price in principal_tag]
book_in_store = [stock.find('p',{'class':'instock availability'}).get_text().strip() for stock in principal_tag]
